...
public class ComByteManager {
    ...
    public final static byte PAL_OTA_CMD = 0x01;            //固件升级命令
    public final static byte PAL_SAM_V_INIT_COM = 0x32;            /*解析服务器开始请求解析命令*/
    public final static byte PAL_SAM_V_APDU_COM = 0x33;            /*解析服务器APDU命令*/
    public final static byte PAL_SAM_V_LOCAL_COM = 0x34;           /*本地安全模块解码命令*/
    ...
    public final static byte ISO14443_P4 = 0x76;
    ...
    public final static byte MAX_FRAME_LEN = 63;
    ...
    public boolean getCmdRunStatus() {
        return (RcvComRunStatus[0] == (byte) 0x90) && (RcvComRunStatus[1] == 0x00);
    }
    ...
    //接收数据处理
    public int rcvData(final byte[] bytes) {
        synchronized (this) {
            int this_frame_num = 0;
            int status = 0;

            if ((bytes == null) || (bytes.length == 0)) {
                return Rcv_Status_Idle;
            }
            ...
            if ((bytes[0] & 0xC0) == 0x00) {     //开始帧
                ...
                RcvData = new byte[RcvFrameNum * (MAX_FRAME_LEN - 1) + RcvDataLen];
                ...
                if (this_frame_num != (last_frame_num + 1)) {        //帧序号不对
                    ...
                    if ( (RcvData == null) || (RcvData.length < (RcvDataLen + bytes.length - 1)) ) {
                        status = Rcv_Status_Idle;
                    } else {
    ...
    public byte[] data_framing_full(byte command, byte[] pSend_data, int send_data_len) {
        ...
        if (send_data_len <= (MAX_FRAME_LEN - 2)) {
            ...
            if (((send_data_len - (MAX_FRAME_LEN - 2)) % (MAX_FRAME_LEN - 1)) > 0) {
                returnFrame = new byte[frame_num * MAX_FRAME_LEN/*中间帧*/ + MAX_FRAME_LEN/*第一帧*/ + ((send_data_len - (MAX_FRAME_LEN - 2)) % (MAX_FRAME_LEN - 1)) + 1/*最后一帧*/];
                frame_num++;
            } else {
                returnFrame = new byte[frame_num * MAX_FRAME_LEN/*后续帧*/ + MAX_FRAME_LEN/*第一帧*/];
            }
        }
    ...
    public byte[] iso15693ReadSingleBlockCmdBytes(byte uid[], byte addr) {
        ...
        System.arraycopy(uid, 0, dataBytes, 0, 8);
        dataBytes[8] = addr;
    ...
    public byte[] iso15693WriteMultipleBlockCmdBytes(byte uid[], byte addr, byte number, byte writeData[]) {
        ...
        byte[] dataBytes = new byte[10 + writeData.length];
        System.arraycopy(uid, 0, dataBytes, 0, 8);
        dataBytes[8] = addr;
        dataBytes[9] = (byte) ((number & 0xff) - 1);
        System.arraycopy(writeData, 0, dataBytes, 10, writeData.length);
    ...
    //获取服务器解析DN码
    public byte[] getSamVInitCmdBytes() {
        //return new byte[] {0x00, PAL_SAM_V_INIT_COM};
        return data_framing_full(PAL_SAM_V_INIT_COM, null, 0);
    }

    //获取服务器解析APDU
    public byte[] getSamVApduCmdBytes(byte[] adpuCrypt) {
        return data_framing_full(PAL_SAM_V_APDU_COM, adpuCrypt, adpuCrypt.length);
    }

    //升级命令
    public byte[] getOTACmdBytes(byte[] OTAData) {
        return data_framing_full(PAL_OTA_CMD, OTAData, OTAData.length);
    }

    //获取本地身份证解析命令
    public byte[] getSamVLocalCmdBytes(byte[] cmd) {
        return data_framing_full(PAL_SAM_V_LOCAL_COM, cmd, cmd.length);
    }
}
