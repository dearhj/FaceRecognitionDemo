...
public class Mifare extends Card {
    ...
    public boolean authenticate(byte bBlockNo, byte bKeyType, byte[] pKey) throws CardNoResponseException {
        ...
        try {
            boolean isSuc = semaphore.tryAcquire(CAR_NO_RESPONSE_TIME_MS, TimeUnit.MILLISECONDS);
            if ( !isSuc ) {
...
//        });
    ...
    /**
     * Perform MIFARE(R) Increment Transfer command sequence with MIFARE Picc.同步阻塞方式
    ...
    /**
     * Perform MIFARE(R) Decrement Transfer command sequence with MIFARE Picc.同步阻塞方式
     * @param bSrcBlockNo  block number to be incremented.
     * @param bDstBlockNo  block number to be transferred to.
     * @param pValue  pValue[4] containing value (LSB first) to be incremented on the MIFARE(R) card
     * @return         true：操作成功  false：操作失败
    ...
    /**
     * Perform MIFARE(R) Restore Transfer command sequence with MIFARE Picc.同步阻塞方式
     * @param bSrcBlockNo  block number to be incremented.
     * @param bDstBlockNo  block number to be transferred to.
     * @return         true：操作成功  false：操作失败
     * @throws CardNoResponseException
    ...
    */
    ...
    /**
     * Perform MIFARE(R) Read Value command with MIFARE Picc.同步阻塞方式
    ...
    /**
     * Perform MIFARE(R) Write Value command with MIFARE Picc.同步阻塞方式
